...
К системе Mindstream относятся контуры формирования корпуса смысловых данных и контуры персональной проекции и внимания в пользовательском контексте браузера.

К внешней среде относятся источники публикаций, LLM API и Embedding API как внешние вычислительные сервисы, а также сеть и инфраструктура выполнения.
...
Подготовка включает извлечение значимого текста и его смысловую нормализацию как предпосылку формирования аннотаций и обзоров.
...
Эти представления не пересматриваются реактивно и используются как базовые смысловые артефакты системы.

**Embedding-контур (`Embedding`)**

Формирует эмбеддинги исключительно для аннотаций и обзоров публикаций.

Эмбеддинги рассматриваются как архитектурные производные смысловых представлений и используются исключительно для смыслового сопоставления публикаций и формирования персональной проекции интересов пользователя.

**Контур хранения корпуса и статистики (`Storage`)**

Обеспечивает хранение публикаций, их смысловых представлений, эмбеддингов и агрегированной статистики внимания.

Состояние данного контура является каноническим и инвариантным по отношению к отдельному пользователю и не пересчитывается в ответ на локальные пользовательские действия.
...
Фиксирует факты пользовательского внимания к публикациям в браузерном контексте.

Сигналы внимания могут использоваться локально и, при соблюдении условий режима доступа, передаваться в серверный контур хранения исключительно как статистические данные.

Архитектурный слой фиксирует сигналы внимания как статистические факты без интерпретации их порядка, а последовательность форм внимания относится к композиционному уровню.
...
- локальные статистические зависимости между сигналами внимания и персональной лентой в браузерных контурах;
...
- архитектурный слой не интерпретирует причинность и порядок возникновения сигналов внимания;
