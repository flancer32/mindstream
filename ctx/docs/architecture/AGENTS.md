...
- `ingestion/` — архитектурные документы контура Ingestion и подготовки публикаций.
- `representations/` — архитектурные представления и их границы.
- `runtime/` — документация архитектурного runtime и его границ.
- `web/` — архитектурные документы web-контура и его ingress-инвариантов.
- `AGENTS.md` — текущий документ, определяющий границы слоя.
- `cli-ingress.md` — архитектурная граница CLI ingress и её инварианты.
- `data-flow.md` — обзор архитектурных потоков данных.
- `http-ingress.md` — архитектурная граница HTTP ingress и её инварианты.
...
В связке серверных и фронтенд-контуров допускаются обратные потоки статистики внимания, не влияющие на формирование корпуса и не нарушающие однонаправленность серверного pipeline.
