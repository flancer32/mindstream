...
- `Db/Schema/Renew.mjs` — leaf-команда `db:schema:renew` для полного пересоздания схемы с best-effort переносом данных.
...
- `Process/` — поддерево команд `process:*`, реализуемое листовыми командами и локальными диспетчерами.
- `Process.mjs` — диспетчер поддерева `process:*` в соответствии с моделью CLI.
- `Process/Generate/` — пространство команд `process:generate:*` (листья дерева).
- `Process/Generate/Summaries.mjs` — leaf-команда `process:generate:summaries` для генерации обзоров и аннотаций.
